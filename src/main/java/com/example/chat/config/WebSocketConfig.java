...
import com.example.global.interceptor.JwtHandshakeInterceptor;
...
public class WebSocketConfig implements WebSocketMessageBrokerConfigurer {
	...
	public WebSocketConfig(
		...
		@Value("${app.cors.allowed-origins:http://localhost:63342}") String allowedOrigins
	) {
	...
	public void registerStompEndpoints(StompEndpointRegistry registry) {
		...
		registry.addEndpoint("/ws/chat")
			...
			.withSockJS()
			.setHeartbeatTime(25000) // 25초마다 heartbeat 전송
			.setDisconnectDelay(5000); // 연결 해제 지연 시간 5초
	}
	...
	public void configureMessageBroker(MessageBrokerRegistry registry) {
		...
		registry.enableSimpleBroker("/sub/ws/chat");
	}
	...
	public void configureWebSocketTransport(WebSocketTransportRegistration registration) {
		registration.setMessageSizeLimit(64 * 1024) // 64KB 메시지 크기 제한
			...
			.setSendTimeLimit(20000); // 20초 전송 타임아웃
	}

	@Override
	public void configureClientInboundChannel(ChannelRegistration registration) {
		registration.interceptors(new com.example.chat.config.StompChannelInterceptor());
	}
