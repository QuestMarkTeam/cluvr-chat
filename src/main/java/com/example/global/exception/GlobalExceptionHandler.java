...
public class GlobalExceptionHandler {
    ...
    public Object handleDataAccessException(HttpServletRequest request, DataAccessException e) {
        ...
        return ResponseEntity
            ...
            .body(BaseResponse.error(ResponseCode.DB_FAIL, ResponseCode.DB_FAIL.getDefaultMessage()));
    }
    ...
    /**
     * NoResourceFoundException 처리 핸들러.
     * 정적 리소스를 찾을 수 없을 때 발생하는 예외를 처리.
     * Chrome DevTools 관련 요청 등은 무시하고 404 응답을 반환.
     *
     * @param request   HTTP 요청 정보
    ...
    public Object handleNoResourceFoundException(HttpServletRequest request, NoResourceFoundException e) {
        ...
        // Chrome DevTools 관련 요청은 로그 레벨을 낮춰서 처리
        if (request.getRequestURI().contains(".well-known") || 
            request.getRequestURI().contains("chrome") ||
            request.getRequestURI().contains("devtools")) {
            log.debug("DevTools resource not found: {}", request.getRequestURI());
        } else {
        ...
        return ResponseEntity
            ...
            .body(BaseResponse.error(ResponseCode.NOT_FOUND, "Resource not found"));
    }
