package com.example.global.response.response;
...
public enum ResponseCode {
    ...
    TOKEN_BLACKLISTED(HttpStatus.FORBIDDEN, "다시 로그인해주세요."),
    NO_JWT(HttpStatus.NOT_FOUND,"JWT 인증정보가 없습니다."),

    /* 서버 */
    UNKNOWN_ERROR(HttpStatus.INTERNAL_SERVER_ERROR, "알 수 없는 오류"),

    FAIL_SUB_TO_USERID(HttpStatus.UNAUTHORIZED,"sub로 userId 조회 실패"),
    FAIL_TO_USERID(HttpStatus.UNAUTHORIZED,"userId 조회 실패: 응답에 data 없음");

    private final HttpStatus status;
    private final String defaultMessage;

    ResponseCode(HttpStatus status, String defaultMessage) {
        this.status = status;
